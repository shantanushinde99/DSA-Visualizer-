...
export function useDijkstra() {
  ...
  const addNode = (x: number, y: number, customId?: string) => {
    // Generate a user-friendly ID (A, B, C, etc.)
    const defaultId = String.fromCharCode(65 + graph.nodes.length) // 65 is 'A'
  ...
  const addEdge = (source: string, target: string, weight: number) => {
    ...
    const sourceExists = graph.nodes.find(n => n.id === source)
    const targetExists = graph.nodes.find(n => n.id === target)
    
    if (!sourceExists || !targetExists) {
      console.warn(`Cannot add edge: one or both nodes don't exist (${source}, ${target})`)
      return false
    }
    ...
    setGraph(prev => ({
      ...
      edges: [
        ...
        // Add reverse edge automatically for undirected graph
  ...
  return {
    ...
    isAutoPlaying,
    toggleAutoPlay,
    steps, // Expose steps for debugging
  }
} 